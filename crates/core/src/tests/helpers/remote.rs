...
use surfpool_types::{DEFAULT_MAINNET_RPC_URL, sanitized_datasource_url};
...
pub async fn fetch(addresses: &[Pubkey]) -> Vec<Account> {
    let endpoint = sanitized_datasource_url(&url()).unwrap_or_else(|| "the datasource".to_string());
    ...
    let results = loop {
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                if errors.len() > 4 {
                    panic!("failed to fetch {addresses:?} from {endpoint}: {errors:#?}");
                }
    ...
    results
        ...
        .map(|(result, address)| {
            result.map_account().unwrap_or_else(|_| {
                panic!(
                    "{address} does not exist at {endpoint}; check {RPC_URL_ENV} points at the cluster \
