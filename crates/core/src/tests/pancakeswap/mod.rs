//! PancakeSwap CLMM integration tests. They fetch real mainnet accounts, so they need a network
...
async fn real_mainnet_pool_round_trips_unchanged() {
    ...
    let template = registry
        .get(PANCAKESWAP.pool_state_template)
...
async fn every_amm_config_index_option_is_a_live_amm_config_account() {
    ...
    let template = registry
        ...
        .expect("template pancakeswap-clmm-amm-config should exist");
...
async fn price_shock_on_the_live_pool_finds_its_tick_array() {
    ...
    let probe = plan_price_shock(PANCAKESWAP, SOL_USDC_POOL, &pool_account, 1.0001)
    ...
    let plan =
        plan_price_shock(PANCAKESWAP, SOL_USDC_POOL, &pool_account, price_factor).expect("plan");
    assert_eq!(
    ...
    assert_eq!(
        tick_array.owner, PANCAKESWAP.program_id,
