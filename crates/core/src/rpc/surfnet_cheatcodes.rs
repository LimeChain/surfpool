            ...
            let (final_account_bytes, final_lamports) = if let Some(conf) = &confidential {
                ...
                let lamports = is_native_mint.then(|| {
                    svm_locker.with_svm_reader(|svm_reader| {
                        svm_reader.inner.minimum_balance_for_rent_exemption(data.len())
...
mod tests {
    ...
    async fn test_set_native_token_2022_amount_keeps_extended_account_rent_exempt() {
        ...
        let base_rent = client.context.svm_locker.with_svm_reader(|svm_reader| {
            svm_reader.inner.minimum_balance_for_rent_exemption(
                TokenAccount::get_packed_len_for_token_program_id(&token_program),
            )
        });
        ...
        let mut data = vec![0; account_len];
        {
            ...
            state.base = Token2022Account {
                ...
                delegate: COption::None,
                state: AccountState::Initialized,
                is_native: COption::Some(base_rent),
                delegated_amount: 0,
                close_authority: COption::None,
            };
            state.pack_base();
