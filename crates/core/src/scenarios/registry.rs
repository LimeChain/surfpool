...
mod tests {
    ...
    fn test_registry_loads_all_protocols() {
        ...
        assert!(registry.contains("meteora-dlmm-custom"));
    ...
    fn test_list_all_template_ids() {
        ...
        assert!(ids.contains(&"meteora-dlmm-custom".to_string()));
    ...
    /// `bin_step` is the base of the bin price math and a seed of the pool's own address, so it is
    /// not a lever and must stay out of the exposed properties.
    #[test]
    fn meteora_templates_expose_the_economic_levers() {
        let registry = TemplateRegistry::new();

        let template = registry
            .get("meteora-dlmm-custom")
            .expect("meteora-dlmm-custom should exist");
