...
pub const HUMIDIFI_V1_OVERRIDES_CONTENT: &str =
    include_str!("./protocols/humidifi/v1/overrides.yaml");
...
impl TemplateRegistry {
    ...
    pub fn load_humidifi_overrides(&mut self) {
        self.load_raw_layout_overrides(HUMIDIFI_V1_OVERRIDES_CONTENT, "humidifi");
    }
...
mod tests {
    ...
    fn test_registry_loads_all_protocols() {
        ...
        assert!(registry.contains("humidifi-fair-value"));
        assert!(registry.contains("humidifi-freshness"));
        assert!(registry.contains("humidifi-stale-quote"));
