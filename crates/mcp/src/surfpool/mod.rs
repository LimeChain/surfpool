...
use surfpool_core::{
    scenarios::{
        TemplateRegistry,
        protocols::{
            meteora::dlmm::v1::price_shock_builder::{
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct CreateMeteoraPriceShockScenarioParams {
    #[schemars(
        description = "Base58 address of the Meteora DLMM pool account, the LbPair owned by LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo."
    )]
    pub pool: String,
    #[schemars(
...
impl Surfpool {
    ...
    #[tool(
        description = "Creates an editable Meteora DLMM price shock scenario: moves the pool's active_id by priceFactor. If validation fails, report that error and do not retry. The backend validates the factor, the pool's owner and size, and that the bin array covering the new active bin already exists."
    )]
    async fn create_meteora_price_shock_scenario(
        &self,
        Parameters(params): Parameters<CreateMeteoraPriceShockScenarioParams>,
    ) -> Result<CallToolResult, McpError> {
            ...
            .await
        {
            Ok(accounts) => match accounts.into_iter().next().flatten() {
                ...
                None => {
                    return Ok(scenario_tool_error(format!(
                        "Meteora DLMM pool {pool} was not found"
                    )));
                }
            },
            Err(error) => return Ok(scenario_tool_error(error)),
        ...
        // The bin array's address falls out of the shocked active id, so it can only be read after the pool.
        let bin_array = match self
            .fetch_surfnet_accounts(params.surfnet_port, &[plan.bin_array])
            .await
        {
            Ok(accounts) => accounts.into_iter().next().flatten(),
            Err(error) => return Ok(scenario_tool_error(error)),
        };
        let scenario = match build_price_shock_scenario(plan, bin_array.as_ref()) {
...
mod tests {
    ...
    #[test]
    fn meteora_price_shock_params_are_camel_case_on_the_wire() {
        let schema =
            serde_json::to_value(schemars::schema_for!(CreateMeteoraPriceShockScenarioParams))
                .expect("schema");
        let properties = schema["properties"]
            .as_object()
            .expect("schema has properties");
        ...
        assert!(
            serde_json::from_value::<CreateMeteoraPriceShockScenarioParams>(serde_json::json!({
                "pool": "BGm1tav58oGcsQJehL9WXBFXF7D27vZsKefj4xJKD5Y",
                "priceFactor": "1.1",
            }))
            .is_ok(),
            "surfnetPort is optional"
        );
    }

    #[tokio::test]
    async fn meteora_price_shock_rejects_invalid_inputs_before_rpc() {
        let surfpool = Surfpool::new();
        ...
        for (pool, price_factor, expected) in [
            ...
            let result = surfpool
                .create_meteora_price_shock_scenario(Parameters(
                    CreateMeteoraPriceShockScenarioParams {
                        pool: pool.to_string(),
